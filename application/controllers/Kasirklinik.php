<?php
class Kasirklinik extends CI_Controller{
  ...
  function index()
  {
    ...
    $query_data="SELECT
                    ...
                    trans_wo.status_wo,
                    trans_wo.status_resep,
                    trans_wo.tipe_wo,
                    trans_wo.id_poli,
                    ms_pasien.alamat,
                    judul_ro.no_bukti AS no_resep,
                    judul_si.no_bukti AS no_penjualan,
                    trans_wo.status_ar,
                    trans_wo.tgl_masuk
                    FROM
                    trans_wo
                    ...
                    INNER JOIN judul_ro ON trans_wo.no_wo = judul_ro.no_ref
                    INNER JOIN judul_si ON judul_ro.no_bukti = judul_si.no_ref
                    WHERE
                    trans_wo.status_wo = 2 AND
                    trans_wo.status_resep = 2";
                            // AND trans_wo.status_wo IS NULL

     // echo $query_wo;
    $data=array(
            'perintah'=>'Baru',
            'title'=>'Daftar Kasir Klinik',
            'title_filter'=>'Cari Kunjungan',
            'title_tambah'=>'Daftar Tagihan Tindakan dan Resep',
     ...
     $this->load->view('Kasirklinik_view',$data);
  }
