...
class Stockopname extends CI_Controller
{
  ...
  function index()
  {
    ...
    $query_data="SELECT
                judul_opname.no_bukti,
                judul_opname.tgl_trans,
                armaster.npa,
                armaster.nama AS nama_customer,
                judul_opname.jenis_bayar,
                judul_opname.jenis_ppn,
                judul_opname.diskon,
                judul_opname.dpp,
                judul_opname.ppn,
                judul_opname.total,
                judul_opname.subtotal,
                judul_opname.voucher,
                judul_opname.jml_cicilan,
                judul_opname.entry_user,
                judul_opname.entry_date,
                a.nama AS nama_pegawai,
                judul_opname.kd_sub_unit_anggota,
                judul_opname.kd_sub_unit,
                c.kd_unit,
                ...
                FROM
                judul_opname
                LEFT JOIN armaster ON judul_opname.id_customer = armaster.npa
                LEFT JOIN mastersubunit AS c ON judul_opname.kd_sub_unit_anggota = c.kd_sub_unit
                LEFT JOIN `user` ON judul_opname.entry_user = `user`.id_user
                LEFT JOIN ms_pegawai AS a ON `user`.id_pegawai = a.id_pegawai
                LEFT JOIN masterunit AS `x2` ON c.kd_unit = `x2`.kd_unit
                LEFT JOIN mastersubunit AS cd ON judul_opname.kd_sub_unit = cd.kd_sub_unit
                LEFT JOIN masterunit AS re ON cd.kd_unit = re.kd_unit
                WHERE
                cd.kd_unit = '$kd_sub_unit'";


            $data=array(
                'title'=>'Data Stock Opname',
                'xmenu'=>'Akuntansi',
                'xsubmenu'=>'Stock Opname',
  ...
  function ambil_data(){
    ...
    function tambah()
    {
      ...
      $this->Bis_model->insertData('judul_opname',$data);
  		...
  		foreach ($this->input->post('rowsBM') as $key => $count )
      {
          ...
          $this->Bis_model->insertData('detail_opname',$data);
      }
  ...
  function hapus()
  {
    $id['no_bukti'] = $this->uri->segment(3);
    $this->db->trans_begin();
    $this->Bis_model->deleteData('judul_opname',$id);
    $this->Bis_model->deleteData('detail_opname',$id);
    if ($this->db->trans_status() === FALSE)
    {
            $this->db->trans_rollback();
    }
...
function cetak()
    {
      ...
      $id = get_cookie('eklinik');
 	    ...
 	    $query_data="SELECT
                  ...
                  LEFT JOIN mastersubunit AS cd ON judul_opname.kd_sub_unit = cd.kd_sub_unit
                  LEFT JOIN masterunit AS re ON cd.kd_unit = re.kd_unit
                  WHERE judul_opname.no_bukti  = '$no_bukti'";
         //echo($query_data);
                      //$query_driver="SELECT * from ms_pegawai where id_jabatan=40";
          $q_detail="SELECT
                      detail_opname.kd_barang,
                      detail_opname.hj,
                      detail_opname.dpp,
                      detail_opname.nilaippn,
                      detail_opname.qty,
                      detail_opname.diskon,
                      detail_opname.perc_diskon,
                      detail_opname.total,
                      detail_opname.satuan,
                      ms_barang.barcode,
                      detail_opname.nama_barang,
                      judul_opname.no_bukti
                      FROM
                      judul_opname
                      INNER JOIN detail_opname ON judul_opname.no_bukti = detail_opname.no_bukti
                      INNER JOIN ms_barang ON detail_opname.kd_barang = ms_barang.id_barang
                      WHERE
                      judul_opname.no_bukti = '$no_bukti'";

	    $data['data_si'] = $this->Bis_model->manualQuery($query_data);
      $data['detail_opname'] = $this->Bis_model->manualQuery($q_detail);
