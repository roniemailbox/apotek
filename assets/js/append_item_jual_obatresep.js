...
jq_append(document).ready(function() {
  ...
  var id_barang = jq_append('#id_barang');
	...
	var keterangan = jq_append('#keterangan');
...
if (x =='' || y=='' || y==0) {
	...
	jq_append('#containeritem').append(
		'<tr class="records">'
			...
			+ '<td>'
				+ '<input class="form-control" style="width:100%; text-align:left" id="satuan_' + count + '" name="satuan_' + count + '" type="text" value="'+ satuan[0].value +'" readonly />'
			+ '</td>'

			+ '<td>'
				+ '<span class="nilaiQty" hidden>'+ qty_barang[0].value +'</span>'
				+ '<input class="form-control" style="width:100%; text-align:center" id="qty_' + count + '" name="qty_' + count + '" type="number" value="'+ qty_barang[0].value +'" onchange="recounter();" onblur="recounter();" readonly />'
			+ '</td>'
			...
			+ '<td>'
				+ '<input class="form-control" style="width:100%; text-align:right" id="keterangan_' + count + '" name="keterangan_' + count + '" type="text" value="" />'
			+ '</td>'
